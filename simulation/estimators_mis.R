...
generate_data_nb <- function(n = 1000, print_summary = FALSE, slider = NULL) {
  ...
  if (is.null(slider)){
    ...
    t <- mapply(function(x, y, z) sample(c(1, 2, 3), size = 1, replace = TRUE, prob = c(x, y, z)), # nolint: line_length_linter.
                p_1, p_2, p_3)
...
calculate_ipw_nb <- function(df, f) {
  ...
  weights <- all_weights[cbind(seq_len(nrow(all_weights))), df$t[w]]
